/*
 *  reso/unit.cc — logue-sdk callback surface for the DEL FX slot.
 */
...
static DelFx s_fx;
static int32_t s_cached[UNIT_DELFX_MAX_PARAM_COUNT];
...
__unit_callback int8_t unit_init(const unit_runtime_desc_t *desc) {
  ...
  for (uint8_t id = 0; id < UNIT_DELFX_MAX_PARAM_COUNT; ++id) {
...
__unit_callback void unit_set_tempo(uint32_t tempo) { s_fx.setTempo(tempo); }
