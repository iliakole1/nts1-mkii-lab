/*
 *  drive/unit.cc — logue-sdk callback surface for the MOD FX slot.
 */
...
static ModFx s_fx;
static int32_t s_cached[UNIT_MODFX_MAX_PARAM_COUNT];
...
__unit_callback int8_t unit_init(const unit_runtime_desc_t *desc) {
  ...
  /* This unit needs no SDRAM; only ask when a buffer is actually wanted. */
  float *buffer = nullptr;
  if (s_fx.getBufferSize() > 0) {
  ...
  for (uint8_t id = 0; id < UNIT_MODFX_MAX_PARAM_COUNT; ++id) {
