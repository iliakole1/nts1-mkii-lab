/*
 *  wt/osc.h — logue-sdk Processor adapter around WtEngine.
 */
#pragma once
...
/*
 * The wave bank. Oscillators get no SDRAM, so it comes from here — and it is
 * held outside the engine on purpose: as a member it would be emitted as
 * initialised .data and spend 16 KB of the budget on zeros. Zero-initialised
 * at namespace scope it lands in .bss and costs nothing in the file.
 */
static int8_t s_table[WtEngine::kBufferSize];
...
class Osc : public Processor {
 public:
  ...
  void init(float *) override final {
    WtEngine::buildTable(s_table);
    engine_.init(s_table);
  }
  ...
  void setShapeLfo(float lfo) { engine_.setWaveMod(lfo); }
